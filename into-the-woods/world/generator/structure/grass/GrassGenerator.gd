...
const GrassStructure = preload("res://world/structure/grass/Grass.tscn")
...
func _is_plant_at(x):
	...
	if top != terrain_map.dirt and top != terrain_map.grass:
		return false
	if forest_info.is_forest(x) or terrain_info.has_water(x):
		return false	# don't spawn in forest or underwater
